...
export class ConferenceUsersTableComponent implements OnInit {
  ...
  initRoleModal(user: BriefUserRoles) {
    ...
    outputs.onModalSubmitted.subscribe((data: BriefUserRoles) => {

      const index:any = this.page.content.indexOf(this.page.content.find(value => value.id === data.id) as BriefUserRoles);
