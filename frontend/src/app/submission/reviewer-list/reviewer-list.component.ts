...
export class ReviewerListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.form.controls['search'].valueChanges.pipe(debounceTime(500)).subscribe(() => this.updateSearchData());
  }
  ...
  deleteReviewers(u: BriefUser) {
    this.submissionService.deleteReviewers(this.submissionId, [u!.id as number])
